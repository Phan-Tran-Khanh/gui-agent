...
# Last ADB error message (set by run_adb_command / execute_action on failure)
_last_adb_error: str = ""


def get_last_adb_error() -> str:
    """Return the most recent ADB failure message, or empty string if none."""
    return _last_adb_error


def _set_last_adb_error(message: str) -> None:
    global _last_adb_error
    _last_adb_error = message


def _clear_last_adb_error() -> None:
    global _last_adb_error
    _last_adb_error = ""
...
def run_adb_command(
    ...
    try:
        ...
        # Check result
        if result.returncode == 0:
            _clear_last_adb_error()
            logger.debug("ADB command succeeded")
            return True, result.stdout.strip()
        else:
            error_msg = result.stderr.strip() or "Command failed"
            cmd_label = " ".join(cmd_args[:3])
            _set_last_adb_error(f"{cmd_label}: {error_msg}")
            logger.warning("ADB command failed: %s", error_msg)
            return False, error_msg

    except subprocess.TimeoutExpired:
        error_msg = f"ADB command timeout (>{timeout}s)"
        _set_last_adb_error(error_msg)
        logger.error(error_msg)
        return False, error_msg
...
def click(
    ...
    if not isinstance(x, int) or not isinstance(y, int):
        _set_last_adb_error(f"click: invalid coordinates ({x}, {y})")
...
def execute_action(
    ...
    try:
        if action_type == "click":
            ...
            _set_last_adb_error(f"unknown action type: {action_type}")
            logger.error("Unknown action type: %s", action_type)
            return False

    except (ValueError, KeyError, TypeError) as e:
        _set_last_adb_error(f"{action_type}: {e}")
