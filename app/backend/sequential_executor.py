...
2. Send annotated screenshot to Planner with user goal/prompt
3. If goal not fulfilled, Planner creates action plan
4. Executor takes action, waits, captures new screenshot
5. Reflect: detect wrong action (visual diff / achiever redo), navigate back if needed
...
# from mllm import AssistantAgent, Plan, PlannerAgent
# # from planning.planner import achieve
# from planner.differentiator import detect_visual_change_from_bytes
# from planning.constraint_retriever import ConstraintRetriever
# from planning.context_retriever import ContextRetriever
# from planning.planner import Planner
# from grounder.annotator import draw_parsed_elements
...
#     5. Planner uses element data to decide which coordinates to click
#     6. Check if goal is achieved using GoalCompletionChecker (AssistantAgent)
#     7. If action needed: Executor performs it, waits, captures screenshot
#     8. Reflect: if wrong action detected, ADB back and trim action history
#     9. Go to step 1 (loop until goal achieved or max steps)
#     """
...
#             self.logger.info("Capturing initial screenshot from device...")
#             if self.mock:
#                 current_screenshot = self._mock_screenshot()
#                 cap_err = None
#             else:
#                 current_screenshot, cap_err = self._capture_screenshot_from_device()
#             if current_screenshot is None:
#                 err_msg = f"adb:screenshot:{cap_err or 'failed'}"
#                 self.logger.error(err_msg)
#                 result.errors.append(err_msg)
#                 result.completion_message = err_msg
...
#         # Action history for reflection (achiever redo detection)
...
#                 subtask = current_milestone.subtasks[0]
#                 action_screenshot_before = current_screenshot
...
#                                 f"{subtask.action_hint.value}: {subtask.description}"
#                             )
...
#                             err_msg = f"adb:{action_type}:{adb.get_last_adb_error() or 'execution failed'}"
#                             self.logger.error(err_msg)
#                             step_data.error = err_msg
#                             result.errors.append(err_msg)
#                     else:
#                         err_msg = f"adb:validate:{validation_error}"
#                         self.logger.error(err_msg)
#                         step_data.error = err_msg
#                         result.errors.append(err_msg)
#                 else:
...
#                 self.logger.info("Capturing next screenshot...")
#                 if self.mock:
#                     screenshot_after_action = self._mock_screenshot()
#                     cap_err = None
#                 else:
#                     screenshot_after_action, cap_err = self._capture_screenshot_from_device()
#                 if screenshot_after_action is None:
#                     error_msg = f"adb:screenshot:{cap_err or 'failed'}"
#                     self.logger.error(error_msg)
#                     step_data.error = error_msg
#                     result.errors.append(error_msg)
#                     current_screenshot = action_screenshot_before
#                 else:
#                     current_screenshot = screenshot_after_action

#                 # ====================================================================
#                 # PHASE 7: REFLECT — detect wrong action, navigate back if needed
#                 # ====================================================================
#                 if (
#                     action_dict
#                     and step_data.action_executed
#                     and action_screenshot_before is not None
#                     and current_screenshot is not None
#                 ):
#                     self.logger.info("")
#                     self.logger.info("PHASE 7: Reflecting on last action...")
#                     self.logger.info("-" * 80)

#                     reflection = await self._reflect_after_action(
#                         user_goal=user_goal,
#                         task_id=task_id,
#                         step_count=step_count,
#                         executed_actions=executed_actions,
#                         action_type=step_data.action_executed,
#                         action_screenshot_before=action_screenshot_before,
#                         screenshot_after_action=current_screenshot,
#                         subtask=subtask,
#                     )
#                     step_data.metadata["reflection"] = reflection

#                     if reflection.get("recovered"):
#                         current_screenshot = reflection.get("screenshot_after_recovery")
#                         if current_screenshot is None:
#                             current_screenshot = screenshot_after_action
...
#     async def _reflect_after_action(
#         self,
#         *,
#         user_goal: str,
...
#         executed_actions: List[str],
#         action_type: str,
#         action_screenshot_before: bytes,
#         screenshot_after_action: bytes,
#         subtask: Any,
#     ) -> Dict[str, Any]:
#         """
#         Detect whether the last action was wrong and recover via ADB back.

#         Uses the same signals as eval.py:
#         - YDiff visual change when VISION_DIFFERENTIATOR_ENABLED
#         - Achiever redo_from_idx when the screen/history indicates a dead-end path
#         """
#         skip_types = {"navigate_back", "wait"}
#         if action_type in skip_types:
#             return {
#                 "wrong_action": False,
#                 "recovered": False,
#                 "recovery_action": "continue",
#                 "reason": f"skipped_reflection_for_{action_type}",
#             }

#         wrong_action, reason, redo_from_idx = self._detect_wrong_action(
#             user_goal=user_goal,
#             executed_actions=executed_actions,
#             action_screenshot_before=action_screenshot_before,
...
#                 title=f"Step {step_count}: Reflection",
#                 description=(
#                     f"Wrong action detected; navigating back. ({reason})"
#                     if wrong_action
#                     else f"No recovery needed. ({reason})"
#                 ),
#                 subgoal_index=step_count,
#                 confidence=0.85 if wrong_action else 0.7,
#                 metadata=outcome,
#             )
...
#         self.logger.warning("Wrong action detected — %s", reason)

#         if redo_from_idx is not None:
#             del executed_actions[redo_from_idx:]
#         elif executed_actions:
#             executed_actions.pop()

#         if not self.mock:
#             back_ok = adb.navigate_back(self.device_id)
#             if not back_ok:
#                 err_msg = f"adb:navigate_back:{adb.get_last_adb_error() or 'failed'}"
#                 self.logger.error(err_msg)
#                 outcome["recovery_error"] = err_msg
#                 return outcome

#         await asyncio.sleep(1.0)
#         if self.mock:
#             screenshot_after_recovery = self._mock_screenshot()
#         else:
#             screenshot_after_recovery, cap_err = self._capture_screenshot_from_device()
#             if screenshot_after_recovery is None and cap_err:
#                 self.logger.error("adb:screenshot:%s", cap_err)

#         outcome.update(
#             {
#                 "recovered": True,
#                 "recovery_action": "navigate_back",
#                 "screenshot_after_recovery": screenshot_after_recovery,
#             }
#         )
...
#                 stage="reflecting",
#                 event_type="reflection_updated",
#                 title=f"Step {step_count}: Recovered",
#                 description="ADB back executed after wrong action",
#                 subgoal_index=step_count,
#                 confidence=0.9,
#                 metadata=outcome,
#             )
...
#         """Return (wrong_action, reason, optional redo_from_idx)."""
#         reasons: List[str] = []
#         redo_from_idx: Optional[int] = None

#         poi: Optional[Tuple[int, int]] = None
#         if subtask.coordinates and len(subtask.coordinates) >= 2:
#             poi = (int(subtask.coordinates[0]), int(subtask.coordinates[1]))

#         if self.mock:
#             return False, "mock_reflection_skipped", None

#         if self.config.vision_differentiator_enabled:
#             changed = detect_visual_change_from_bytes(
#                 action_screenshot_before,
#                 screenshot_after_action,
#                 poi=poi,
#             )
#             if not changed:
#                 reasons.append("no_visual_change")

#         image = self._screenshot_bytes_to_image(screenshot_after_action)
...
#                 redo_from_idx = ach.redo_from_idx
#                 reasons.append(f"achiever_redo_from_idx={redo_from_idx}")

#         if not reasons:
#             return False, "action_effective", None

#         return True, "; ".join(reasons), redo_from_idx

#     @staticmethod
#     def _screenshot_bytes_to_image(screenshot_bytes: bytes) -> Optional[Image.Image]:
#         try:
...
#     def _capture_screenshot_from_device(self) -> Tuple[Optional[bytes], Optional[str]]:
#         """
#         Capture screenshot from device using ADB.

#         Returns:
#             (screenshot_bytes, error_detail) — error_detail is set only on failure
#         """
#         try:
#             temp_local_path = "output/temp_screenshot.png"
#             device_screenshot_path = "/sdcard/gui_agent_screenshot.png"
...
#             if not success:
#                 return None, f"screencap: {output or adb.get_last_adb_error()}"
...
#             if not success:
#                 return None, f"pull: {output or adb.get_last_adb_error()}"
